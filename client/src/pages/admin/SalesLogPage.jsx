...
export function SalesLogPage() {
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      const mapped = data.map((sale) => ({
        ...
        date: sale.createdAt ? new Date(sale.createdAt).toISOString().slice(0, 10) : new Date().toISOString().slice(0, 10),
        total: sale.total,
  ...
  const saveItem = async (event) => {
    ...
    try {
      const paymentMethod = form.payment.toLowerCase().includes('transfer') ? 'Bank Transfer' : 'Cash'
      if (editingId) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setError(err.message || 'Failed to delete sales record.')
    }
  }
  ...
  return (
    ...
    >
      ...
      {error && (
        <div className="mt-4 bg-red-50 text-red-700 p-4 rounded-xl border border-red-200">
          <p className="font-semibold">{error}</p>
        </div>
      )}

      {loading ? (
        <div className="py-12 text-center text-neutral-500">Loading sales records...</div>
      ) : (
        <>
          ...
          <section className="mt-8 rounded-2xl bg-[#efefef] overflow-x-auto">
            <div className="min-w-[800px]">
              ...
              {items.length === 0 ? (
                <div className="py-12 text-center text-neutral-400">No sales logged yet.</div>
              ) : (
                items.map((item) => {
                  const status = SALES_STATUS_META[item.status] || { label: item.status || 'Complete', className: 'bg-green-100 text-green-700' }
                  ...
                  return (
                    <div key={item.id} className="grid grid-cols-[1fr_1.2fr_0.9fr_0.8fr_1fr_0.9fr] items-center border-b border-neutral-200 px-6 py-4 text-sm last:border-b-0">
                      <span className="font-semibold text-[#ef2027] truncate pr-2">#{item.id.slice(-6).toUpperCase()}</span>
