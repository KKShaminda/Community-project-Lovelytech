    ...
    <article className="group rounded-xl border-t-4 border-red-600 bg-white p-2.5 shadow-sm transition duration-200 hover:-translate-y-1 hover:shadow-md">
      <div className="relative mb-2.5 aspect-[4/3.2] overflow-hidden rounded-lg bg-gray-50">
        ...
        <button
          ...
          className={`absolute right-2 top-2 flex h-7 w-7 items-center justify-center rounded-full border shadow-sm transition-colors ${isWishlisted ? 'border-white bg-[#E4342F]' : 'border-[#E4342F] bg-white'
            }`}
        >
      ...
      <button
        ...
        disabled={outOfStock}
        className={`flex w-full items-center justify-center gap-2 rounded-lg py-2 text-sm font-semibold text-white transition ${justAdded
            ? 'bg-emerald-600'
